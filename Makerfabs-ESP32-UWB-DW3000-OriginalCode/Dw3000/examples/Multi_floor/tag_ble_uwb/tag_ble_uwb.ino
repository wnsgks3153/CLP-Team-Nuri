...
// ESP32 3.2.0 호환을 위한 BLE 헤더 포함 방식 변경
#include <BLEDevice.h>
#include <BLEUtils.h>
#include <BLEScan.h>
...
#define APP_NAME "Multi-Floor UWB Tag with BLE v1.0"
...
// UWB 통신 프레임 설정
const uint16_t TAG_ID = 0xDEAD; // 태그 ID
#define INITIATOR_ID_BYTE_7 (TAG_ID & 0xFF)
#define INITIATOR_ID_BYTE_8 ((TAG_ID >> 8) & 0xFF)
...
struct NearbyAnchor {
  ...
  uint8_t floor;     // 앵커 층 (0=1층, 1=2층, 2=3층...)
  int rssi;          // BLE 신호 강도
  uint32_t lastActivated; // 마지막 활성화 시간 (millis())
};

// 발견된 앵커 목록
std::vector<NearbyAnchor> nearbyAnchors;

// BLE 스캔 콜백 클래스 - ESP32 3.2.0 BLE 라이브러리 호환 수정
class AdvertisedDeviceCallbacks: public BLEAdvertisedDeviceCallbacks {
  public:
    void onResult(BLEAdvertisedDevice advertisedDevice) {
      // 서비스 UUID가 일치하는 경우만 처리
      if (advertisedDevice.haveServiceUUID() && advertisedDevice.isAdvertisingService(BLEUUID(BLE_SERVICE_UUID))) {
        // 앵커 ID 추출 (제조사 데이터)
        if (advertisedDevice.haveManufacturerData()) {
          // ESP32 3.2.0 버전 대응: 데이터 포인터와 길이 직접 가져오기
          std::string manufacturerDataStr = advertisedDevice.getManufacturerData();
          const uint8_t* manufacturerData = (const uint8_t*)manufacturerDataStr.data();
          size_t manufacturerDataLen = manufacturerDataStr.length();
          
          // 데이터 길이가 충분한지 확인 (제조사 ID 2바이트 + 데이터 4바이트)
          if (manufacturerDataLen >= 6) {
            // 제조사 ID (첫 2바이트) 건너뛰고 앵커 데이터 읽기
            // 앵커 ID는 제조사 ID 다음 2바이트에 있음
            uint16_t anchorId = (manufacturerData[2] << 8) | manufacturerData[3];
            // 층 정보는 앵커 ID 다음 1바이트에 있음
            uint8_t floorNum = manufacturerData[4];
            
            int rssi = advertisedDevice.getRSSI();
            
            // RSSI 기반으로 가까운 앵커 판단
            if (rssi > BLE_PROXIMITY_RSSI) {
              Serial.print("Found anchor: 0x");
              Serial.print(anchorId, HEX);
              Serial.print(", Floor: ");
              Serial.print(floorNum);
              Serial.print(", RSSI: ");
              Serial.println(rssi);
              
              // 가까운 앵커 목록에 추가 (이미 존재하지 않는다면)
              bool alreadyExists = false;
              for (int i = 0; i < nearbyAnchors.size(); i++) {
                if (nearbyAnchors[i].id == anchorId) {
                  // 이미 목록에 있으면 RSSI만 업데이트
                  nearbyAnchors[i].rssi = rssi;
                  alreadyExists = true;
                  break;
                }
              }
              ...
              if (!alreadyExists) {
                NearbyAnchor newAnchor;
                newAnchor.id = anchorId;
                newAnchor.floor = floorNum;
                newAnchor.rssi = rssi;
                newAnchor.lastActivated = 0;
                nearbyAnchors.push_back(newAnchor);
              }
            }
          }
        }
...
void scanForNearbyAnchors(); // BLE로 주변 앵커 스캔
void activateNearbyAnchor(uint16_t anchorId); // UWB 거리측정 위해 앵커 활성화 요청
AnchorData* findAnchorById(uint16_t anchorId); // ID로 앵커 정보 찾기
void sortNearbyAnchorsByRSSI(); // RSSI 기준 앵커 정렬
...
void setup() {
  Serial.begin(115200);
  delay(1000); // 시작 안정화
  Serial.println("===== BLE+UWB 멀티 플로어 태그 시작 =====");
  
  // BLE 초기화
  BLEDevice::init("UWB-Tag");
  ...
  while (!dwt_checkidlerc()) {
    Serial.println("IDLE 확인 실패 - 재시도 중");
    delay(100);
  }

  if (dwt_initialise(DWT_DW_INIT) == DWT_ERROR) {
    Serial.println("UWB 초기화 실패 - 중단됨");
    while (1) ; // 중단
  }
  ...
  Serial.println("UWB 초기화 완료");
  Serial.println("시스템 준비 완료. 주변 앵커 스캔 시작...");
}

void loop() {
  // 1. BLE 스캔으로 주변 앵커 탐색
  static uint32_t lastScanTime = 0;
  if (millis() - lastScanTime > BLE_SCAN_TIME * 1000) {
    Serial.println("주변 앵커 BLE 스캔 시작...");
    scanForNearbyAnchors();
    lastScanTime = millis();
  }
  
  // 2. 검출된 가까운 앵커들에 대해 UWB 거리 측정
  int valid_ranges_count = 0;

  // 가장 가까운(RSSI) 앵커 MAX_ACTIVE_ANCHORS개 활성화 및 거리 측정
  sortNearbyAnchorsByRSSI();
  
  int anchorsToMeasure = min((int)nearbyAnchors.size(), MAX_ACTIVE_ANCHORS);
  if (anchorsToMeasure > 0) {
    Serial.print("BLE 발견 앵커 ");
    Serial.print(anchorsToMeasure);
    Serial.println("개와 UWB 거리 측정 시작");
  } else {
    Serial.println("BLE로 발견된 앵커 없음");
  }
  
  for (int i = 0; i < anchorsToMeasure; i++) {
    // 일정 시간이 지나면 앵커 재활성화
    uint32_t currentTime = millis();
    if (currentTime - nearbyAnchors[i].lastActivated > ACTIVE_ANCHOR_TIMEOUT_MS) {
      // 앵커 활성화 요청 (BLE로)
      activateNearbyAnchor(nearbyAnchors[i].id);
      nearbyAnchors[i].lastActivated = currentTime;
      
      // 활성화 후 짧은 지연 (앵커가 UWB를 켤 시간 부여)
      delay(100);
    }
    
    // 앵커 ID로 AnchorData 구조체 찾기
    AnchorData* anchor = findAnchorById(nearbyAnchors[i].id);
    if (anchor == NULL) {
      Serial.print("ERROR: 앵커 ID 0x");
      Serial.print(nearbyAnchors[i].id, HEX);
      Serial.println("에 대한 정보를 찾을 수 없음");
      continue;
    }
    
    // UWB 거리 측정 시작
    Serial.print("앵커 0x");
    Serial.print(anchor->id, HEX);
    Serial.println("과 UWB 거리 측정 시작...");
    
    // 거리 측정 전 현재 앵커의 유효성 초기화
    anchor->distance_valid = false;
    anchor->distance = -1.0;

    // Poll 메시지의 목적지 주소 설정
    tx_poll_msg[ALL_MSG_DEST_ADDR_IDX] = anchor->id & 0xFF; // 하위 바이트
    tx_poll_msg[ALL_MSG_DEST_ADDR_IDX + 1] = (anchor->id >> 8) & 0xFF; // 상위 바이트
    ...
    if (status_reg & SYS_STATUS_RXFCG_BIT_MASK) // 좋은 프레임 수신 성공
    {
      ...
      if (frame_len <= sizeof(rx_buffer)) // 버퍼 크기 확인
      {
        ...
        bool correct_response = (rx_buffer[9] == 0xE1) &&
                                ...
                                (rx_buffer[ALL_MSG_SRC_ADDR_IDX] == (anchor->id & 0xFF)) &&
                                (rx_buffer[ALL_MSG_SRC_ADDR_IDX + 1] == ((anchor->id >> 8) & 0xFF));
        ...
        if (correct_response) // 응답이 유효하면 거리 계산 수행
        {
          ...
          // 현재 앵커에 대한 결과 저장
          anchor->distance = distance_calc;
          anchor->distance_valid = true;
          anchor->last_heard_ts = dwt_readsystimestamphi32(); // 시스템 타임스탬프 상위 비트 저장 (참고용)
        ...
        Serial.print("앵커 0x");
        Serial.print(anchor->id, HEX);
        Serial.print(": 프레임 길이 오류 (");
        Serial.print((unsigned long)frame_len);
        Serial.println(")");
      }
  ...
  // 발견된 앵커가 없으면 직접 모든 앵커와 통신 시도
  if (anchorsToMeasure == 0) {
    Serial.println("BLE 발견 앵커 없음 - 모든 앵커와 직접 UWB 통신 시도");
    
    // BLE 없이 모든 앵커 측정
    for (int i = 0; i < NUM_ANCHORS; i++) {
      // UWB 거리 측정 시작
      Serial.print("앵커 0x");
      Serial.print(anchors[i].id, HEX);
      Serial.println("과 직접 UWB 거리 측정 시도...");
      ...
      if (status_reg & SYS_STATUS_RXFCG_BIT_MASK) // 좋은 프레임 수신 성공
      {
        ...
        if (frame_len <= sizeof(rx_buffer)) {
          ...
          if (correct_response) {
            ...
            tof_calc = ((rtd_init - rtd_resp * (1.0 - clockOffsetRatio)) / 2.0) * DWT_TIME_UNITS;
            distance_calc = tof_calc * SPEED_OF_LIGHT;
            ...
            Serial.print(anchors[i].id, HEX);
            Serial.print(" (층:");
            Serial.print(anchors[i].floor + 1);
            Serial.print("): ");
            Serial.print(anchors[i].distance);
            Serial.println(" m");
            
            // BLE 발견 목록에 추가 (나중에 BLE를 통해 이 앵커 발견 시 유리)
            bool alreadyExists = false;
            for (int j = 0; j < nearbyAnchors.size(); j++) {
              if (nearbyAnchors[j].id == anchors[i].id) {
                alreadyExists = true;
                break;
              }
            }
            ...
            if (!alreadyExists) {
              NearbyAnchor newAnchor;
              newAnchor.id = anchors[i].id;
              newAnchor.floor = anchors[i].floor;
              newAnchor.rssi = -80; // 낮은 RSSI 값 (BLE로 발견된 것이 아니므로)
              newAnchor.lastActivated = millis();
              nearbyAnchors.push_back(newAnchor);
            }
          } else {
            Serial.print("앵커 0x");
  ...
  // 3. 위치 계산
  if (valid_ranges_count >= 3) {
    AnchorData* nearest[3] = {nullptr, nullptr, nullptr};
    findNearestThreeAnchors(nearest);

    if (nearest[0] && nearest[1] && nearest[2]) {
      Serial.println("가장 가까운 앵커 3개 발견. 위치 계산 중...");
      position_valid = calculatePosition(nearest[0], nearest[1], nearest[2], current_position);
...
// 주변 앵커 BLE 스캔
void scanForNearbyAnchors() {
  Serial.println("BLE 스캔 시작...");
  // 이전 스캔 상태 지우기
  // 주의: 앵커의 lastActivated 시간 정보를 유지하기 위해 완전히 지우지는 않음
  for (int i = 0; i < nearbyAnchors.size(); i++) {
    // 마지막 활성화 시간 정보는 유지
    uint32_t lastActTime = nearbyAnchors[i].lastActivated;
    nearbyAnchors[i].rssi = -100; // 낮은 RSSI로 초기화
    nearbyAnchors[i].lastActivated = lastActTime;
  }
  
  // ESP32 3.2.0 BLE 라이브러리 호환성 처리
  ...
  // 스캔 실행
  bool scanStarted = pBLEScan->start(BLE_SCAN_TIME, false);
  if(!scanStarted) {
    Serial.println("BLE 스캔 시작 실패!");
    // 스캔 실패 시 재시도
    delay(100);
    pBLEScan->start(BLE_SCAN_TIME, false);
  }
  
  Serial.print("BLE 스캔 완료, ");
  Serial.print(nearbyAnchors.size());
  Serial.println(" 개의 앵커 등록됨");
}

// 주변 앵커 RSSI 기준 내림차순 정렬
void sortNearbyAnchorsByRSSI() {
  std::sort(nearbyAnchors.begin(), nearbyAnchors.end(), 
    [](const NearbyAnchor& a, const NearbyAnchor& b) {
      return a.rssi > b.rssi; // RSSI 값이 큰 것이 가까움 (더 강한 신호)
    }
  );
}

// BLE 통신으로 앵커 활성화 요청
void activateNearbyAnchor(uint16_t anchorId) {
  // BLE 클라이언트를 사용하여 앵커에 연결하고 UWB 활성화
  // ESP32 3.2.0 BLE 라이브러리 호환 구현
  Serial.print("앵커 0x");
  Serial.print(anchorId, HEX);
  Serial.println(" 활성화 요청 전송");
  
  // 연결할 앵커 이름 생성
  char deviceName[30];
  sprintf(deviceName, "UWB-Anchor-0x%04X", anchorId);
  
  // 로컬 블루투스 클라이언트 객체 생성 - ESP32 3.2.0 호환
  static BLEClient* pClient = BLEDevice::createClient();
  static bool clientBusy = false;
  
  // 이미 다른 요청 처리 중이면 건너뜀
  if (clientBusy) {
    Serial.println("이미 활성화 요청 처리 중입니다. 건너뜀.");
    return;
  }
  
  clientBusy = true;
  
  // 연결 해제 핸들러 클래스 - ESP32 3.2.0 호환
  class ClientCallback : public BLEClientCallbacks {
    void onConnect(BLEClient* pclient) {
      Serial.println("앵커에 연결됨");
    }
    
    void onDisconnect(BLEClient* pclient) {
      Serial.println("앵커에서 연결 해제됨");
      clientBusy = false;
    }
  };
  ...
  // 서비스 및 특성 UUID를 지역 변수로 정의
  static BLEUUID serviceUUID(BLE_SERVICE_UUID);
  static BLEUUID charUUID("beb5483e-36e1-4688-b7f5-ea07361b26a8"); // UWB 제어 특성 UUID
  
  // 스캔된 기기들에서 앵커 ID에 해당하는 기기 찾기
  class FoundDeviceCallback: public BLEAdvertisedDeviceCallbacks {
    public:
      bool found = false;
      BLEAdvertisedDevice* myDevice;
      uint16_t targetAnchorId;
      
      FoundDeviceCallback(uint16_t id) : targetAnchorId(id) {}
      ...
      void onResult(BLEAdvertisedDevice advertisedDevice) {
        if (advertisedDevice.haveServiceUUID() && 
          ...
          if (advertisedDevice.haveManufacturerData()) {
            ...
            if (manufacturerDataLen >= 6) {
              // 앵커 ID 추출 (제조사 ID 다음 2바이트)
              uint16_t anchorId = (manufacturerData[2] << 8) | manufacturerData[3];
              
              // 타겟 앵커 ID와 일치하는지 확인
              if (anchorId == targetAnchorId) {
                myDevice = new BLEAdvertisedDevice(advertisedDevice);
                found = true;
                Serial.print("타겟 앵커 기기 발견: ");
  ...
  // 타겟 앵커 ID로 스캔
  FoundDeviceCallback* targetCallback = new FoundDeviceCallback(anchorId);
  BLEScan* pScan = BLEDevice::getScan();
  pScan->setAdvertisedDeviceCallbacks(targetCallback);
  pScan->setActiveScan(true);
  pScan->start(3); // 3초간 스캔
  
  // 대상 앵커를 찾았는지 확인
  if (!targetCallback->found) {
    Serial.println("타겟 앵커를 찾을 수 없음. UWB 활성화 실패.");
    clientBusy = false;
  ...
  // 앵커에 연결 시도
  if (pClient->connect(targetCallback->myDevice)) {
    Serial.println("앵커에 연결됨! 서비스 찾는 중...");
    
    // 서비스 찾기
    BLERemoteService* pRemoteService = pClient->getService(serviceUUID);
    if (pRemoteService == nullptr) {
      Serial.println("앵커에서 서비스를 찾을 수 없음");
      pClient->disconnect();
      clientBusy = false;
      delete targetCallback;
      return;
    ...
    if (pRemoteCharacteristic == nullptr) {
      Serial.println("앵커에서 특성을 찾을 수 없음");
      pClient->disconnect();
      clientBusy = false;
      delete targetCallback;
      return;
    ...
    // UWB 활성화 명령 전송
    uint8_t activateCmd = 1;
    if (pRemoteCharacteristic->canWrite()) {
      pRemoteCharacteristic->writeValue(&activateCmd, 1);
      Serial.println("UWB 활성화 명령 성공적으로 전송됨");
    } else {
    ...
    Serial.println("앵커에 연결할 수 없음");
    clientBusy = false;
  }
  
  delete targetCallback;
  
  // 시뮬레이션 없이 실제로 BLE 통신이 구현되어도
  // 연결이 실패할 가능성이 있으므로 기본적으로 측정은 계속 수행
}

// ID로 앵커 정보 찾기
AnchorData* findAnchorById(uint16_t anchorId) {
  for (int i = 0; i < NUM_ANCHORS; i++) {
...
bool calculatePosition(AnchorData* a1, AnchorData* a2, AnchorData* a3, Point& result_pos) {
  ...
  if (abs(calc_dist1 - r1) > MAX_ERROR ||
    ...
    Serial.print("삼각측량 경고: 오차 큼 (");
    Serial.print(abs(calc_dist1 - r1));
    Serial.print(", ");
    Serial.print(abs(calc_dist2 - r2));
    Serial.print(", ");
    Serial.print(abs(calc_dist3 - r3));
    Serial.println(")");
    // 경고만 출력하고 계산은 계속 진행
  }
